...
붙여넣으려고 클립보드에 잠깐 넣는 경로에는 "잠깐 쓰는 내용" 표시를 붙여요. 그래서 Raycast 같은 클립보드 기록 앱에 경로가 쌓이지 않습니다.

## Raycast 클립보드 기록에서 붙여넣기

Raycast 클립보드 기록에서 항목을 더블클릭하면(Paste to Ghostty) 이미지와 글자 모두 Ghostty에 들어가요.

원래는 이렇게 하면 한글 입력 상태에서 `ㅍ`만 찍혔어요. Raycast가 흉내 내서 보낸 `Cmd+V`를 Ghostty가 붙여넣기로 알아보지 못하고, 한글 입력기가 `V`를 `ㅍ`로 바꿨기 때문이에요. 이제는 Raycast가 붙여넣으려고 클립보드에 넣은 내용을 이 도구가 받아서 대신 붙입니다. `ㅍ`가 먼저 찍혔으면 지우고 붙여요.

Raycast 스니펫도 같은 방식으로 붙여넣기 때문에, Ghostty에서는 스니펫도 이 도구가 처리해요.
